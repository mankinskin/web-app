...
async fn index() -> impl Responder {
    info!("Index request");
    NamedFile::open(format!("{}/index.html", CLIENT_PATH))
}
#[get("/price_history")]
...
pub async fn run() -> std::io::Result<()> {
    ...
    ssl_builder.set_certificate_chain_file(keys::to_key_path("tls.crt"))?;
    ssl_builder.set_private_key_file(keys::to_key_path("tls.key"), SslFiletype::PEM)?;
    let server = HttpServer::new(move ||
            App::new()
                ...
                .service(ws_route)
                .service(Files::new("/", format!("{}/pkg", CLIENT_PATH)))
        )
