...
impl IndexSplitter {
    pub(crate) fn index_subrange<T: Tokenize + Display>(hypergraph: &mut Hypergraph<T>, root: impl Indexed + Clone, range: impl PatternRangeIndex) -> RangeSplitResult {
        let root = root.index();
        ...
        match Self::verify_range_split_indices(vertex.width, range) {
            DoubleSplitPositions::Double(lower, higher) => {
                ...
                let (left, inner, right) = match Self::build_double_split_kinds(&vertex, patterns, lower, higher) {
                    Ok((pattern_id, pre, left, inner, right, post)) => {
                        // perfect split
                        (
                            Self::resolve_perfect_split_range(hypergraph, pre, root, pattern_id, 0..left),
                            Self::resolve_perfect_split_range(hypergraph, inner, root, pattern_id, left..right),
                            Self::resolve_perfect_split_range(hypergraph, post, root, pattern_id, right..),
                        )
                    },
                    Err(indices) => {
                        // unperfect splits
            ...
            DoubleSplitPositions::None => {
                RangeSplitResult::Full(Child::new(root, vertex.width))
            }
        }
    }
    ...
    pub(crate) fn build_double_split_kinds(
        ...
        match patterns.into_iter().try_fold(vec![], move |mut acc, (pattern_index, pattern)| {
            let left_split = Self::find_pattern_split_index(pattern.clone(), left).expect("left split not in pattern");
            let right_split = Self::find_pattern_split_index(pattern, right).expect("left split not in pattern");
            ...
            match (left, right) {
                (Ok((left, left_ind)), Ok((right, right_ind))) => {
                    ...
                    let new = match right_index - left_index {
                        0 => {
                            ...
                            (pattern_index, DoubleSplitIndex::Inner(prefix, (left.index, left.offset, right.offset), postfix))
                        },
                        _ => {
                            let (prefix, infix, postfix) = double_split_context(pattern, left_index, right_index);
                            (pattern_index, DoubleSplitIndex::Infix(prefix, left, infix, right, postfix))
                        },
                    };
                    acc.push(new);
                    Ok(acc)
                ...
                (Ok((left, left_ind)), Err(right_ind)) => {
                    ...
                    let (infix, postfix) = split_pattern_at_index(&rem, right_index-left_index);
                    let new = (pattern_index, DoubleSplitIndex::Right(prefix, left, infix, right_index, postfix));
                    acc.push(new);
                    Ok(acc)
                },
                (Err(left_ind), Ok((right, right_ind))) => {
                    ...
                    let new = (pattern_index, DoubleSplitIndex::Left(prefix, left_index, infix, right, postfix));
                    acc.push(new);
                    Ok(acc)
                },
                (Err(left_ind), Err(right_ind)) => {
                    ...
                    let (prefix, rem) = split_pattern_at_index(pattern, left_index);
                    let (infix, postfix) = split_pattern_at_index(&rem, right_index-left_index);
                    Err((pattern_index, prefix, left_index, infix, right_index, postfix))
                },
            }
        }) {
    ...
    pub(crate) fn double_split_from_indices<T: Tokenize + Display>(
        ...
        let (left, inner, right) = indices.into_iter()
        .fold((vec![], vec![], vec![]), |(mut la, mut ia, mut ra), (_pattern_id, split_index)| {
            match split_index {
    ...
    pub fn find_single_split_indices(
        ...
        patterns.into_iter().map(move |(i, pattern)| {
            let split = Self::find_pattern_split_index(pattern, pos).expect("Split not in pattern");
            (i, split)
        }).collect()
    }
    pub(crate) fn process_single_splits<T: Tokenize + Display>(
        ...
        let (left, right) = Self::single_split_from_indices(hypergraph, root, perfect_split, remaining_splits);
    ...
    pub(crate) fn separate_single_split_indices(
        ...
        Self::perfect_split_search(current_node, split_indices).into_iter()
    ...
    pub(crate) fn verify_range_split_indices(
        ...
        if let Some(lower) = NonZeroUsize::new(lower) {
            ...
            // lower bound out
            DoubleSplitPositions::Single(NonZeroUsize::new(higher).expect("upper bound is zero dispite check"))
        }
    }
    ...
    pub(crate) fn split_index<T: Tokenize + Display>(hypergraph: &mut Hypergraph<T>, root: impl Indexed, pos: NonZeroUsize) -> (Child, Child) {
        let root = root.index();
    ...
    fn perform_perfect_split<T: Tokenize>(hypergraph: &mut Hypergraph<T>, ((pl, pr), ind): (Split, IndexInParent), parent: impl Indexed) -> (Child, Child) {
        ...
        let parent = parent.index();
        (
            Self::resolve_perfect_split_range(hypergraph, pl, parent, ind.pattern_index, 0..ind.replaced_index),
            Self::resolve_perfect_split_range(hypergraph, pr, parent, ind.pattern_index, ind.replaced_index..)
        )
    }
    ...
    fn perform_child_splits<T: Tokenize + Display>(hypergraph: &mut Hypergraph<T>, child_splits: Vec<SplitContext>) -> (Child, Child) {
        // for every child split
    ...
    pub(crate) fn perfect_split_search<'a>(
        ...
        split_indices.into_iter().map(move |(pattern_index, split_index)| {
            let pattern = current_node.get_child_pattern(&pattern_index).unwrap();
            Self::separate_pattern_split(pattern_index, split_index)
                .map(move |(key, IndexInParent {
    ...
    pub(crate) fn separate_pattern_split(
        ...
        let SplitIndex { index_pos, pos, index } = split_index;
