...
impl<C: Currency> Transaction<C> {
	...
	pub fn get_amount(&self) -> C {
		self.amount.clone()
	}
	pub fn set_amount<Amt: Into<C>>(&mut self, amt: Amt) -> &mut Self {
	...
	pub fn get_sender(&self) -> Subject {
		self.sender.clone()
	}
	pub fn set_sender<S: Into<Subject>>(&mut self, subject: S) -> &mut Self {
		self.sender = subject.into();
		self
	}
	pub fn get_recipient(&self) -> Option<Subject> {
		self.recipient.clone()
	}
	pub fn set_recipient<S: Into<Subject>>(&mut self, subject: S) -> &mut Self {
		self.recipient = Some(subject.into());
		self
	}
	pub fn get_purposes(&self) -> Option<Purposes> {
		self.purposes.clone()
	}
	pub fn set_purposes<P: Into<Purpose> + Clone>(&mut self, purposes: Vec<P>) -> &mut Self {
	...
	pub fn add_purpose<P: Into<Purpose>>(&mut self, purpose: P) -> &mut Self {
	...
	named!(
		...
		map!(
			tuple!(
				...
				opt!(terminated!(DateTime::<Utc>::parse, space1)),
				// <Subject>: I | Name
				preceded!(space0, Subject::parse),
				// <Action>: got, gave, get, give, ...
				preceded!(space1, Action::parse),
				// <Object>: 10 euros, 1€,
				preceded!(space1, Euro::parse),
				// (to <Recipient>): Me | Name
				opt!(preceded!(
					space1,
					preceded!(
						terminated!(alt!(tag_no_case!("to") | tag_no_case!("from")), space1),
						Subject::parse
					)
				)),
				// (for <Purpose>)
				opt!(preceded!(
					delimited!(space1, tag_no_case!("for"), space1),
					Purpose::parse
				))
			),
...
mod tests {
	...
	#[test]
	fn with_recipient() {
		let parsed = Transaction::parse("Today I gave 5€ to Recipient")
			.unwrap()
			.1;
		assert!(
			parsed
