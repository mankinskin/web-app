...
fn command<'a, 'b>(name: impl Into<String>) -> App<'a, 'b> {
    App::new(name).setting(clap::AppSettings::DisableVersion)
}
pub async fn run_command(text: String) -> Result<String, Error> {
    ...
    let app = command("")
        ...
        .subcommand(
            command("watch").arg(Arg::with_name("symbol").help("The Market symbol to watch")),
            )
